...
if (isset($data['current']['stage_id'])) {
    ...
    $sms_seq_data = qs("select * from sms_sequence where tenant_id='".$GLOBALS['tenant_id']."' AND last_deal_id='{$data['current']['id']}'");
    if (isset($deal_info['data']['e585bd988070d2bdfb2af36d968521c3f9aa949a'])) {
        if ($deal_info['data']['e585bd988070d2bdfb2af36d968521c3f9aa949a'] == '196') {
            if(!empty($seq_deal) && $seq_deal['day7_1_sent']=='1' && $seq_deal['day7_1_sent']=='0'){
                qu("deal_seq_status", _escapeArray(array("seq_status" => 'OFF_AUTO')),"tenant_id='".$GLOBALS['tenant_id']."' AND deal_id='{$data['current']['id']}'");            
            }else{
                qu("deal_seq_status", _escapeArray(array("seq_status" => 'OFF_MANU')),"tenant_id='".$GLOBALS['tenant_id']."' AND deal_id='{$data['current']['id']}'");            
            }            
        } else if ($deal_info['data']['e585bd988070d2bdfb2af36d968521c3f9aa949a'] == '195') {
            qu("deal_seq_status", _escapeArray(array("seq_status" => 'ON')),"tenant_id='".$GLOBALS['tenant_id']."' AND deal_id='{$data['current']['id']}'");                        
        }
    }
    
    $sms_seq_app_out_data = qs("select * from sms_sequence_app_out where tenant_id='".$GLOBALS['tenant_id']."' AND last_deal_id='{$data['current']['id']}'");
    if((!empty($sms_seq_data) || !empty($sms_seq_app_out_data)) && !empty($deal_info)){
        if(($sms_seq_data['need_to_send_sms']==1 || $sms_seq_app_out_data['need_to_send_sms']==1) && $deal_info['data']['e585bd988070d2bdfb2af36d968521c3f9aa949a']=='196'){
            $user_detail = qs("select * from pd_users where tenant_id='".$GLOBALS['tenant_id']."' AND pd_id='{$data['meta']['user_id']}'");
    ...
    $email_seq_data = qs("select * from email_sequence where tenant_id='".$GLOBALS['tenant_id']."' AND last_deal_id='{$data['current']['id']}'");
    ...
    if (isset($deal_info['data']['id'])) {
        ...
        addLogs($_REQUEST['q'], $GLOBALS['tenant_id'], "Deal info not found. Detail" . json_decode($deal_info));
        die;
    }
} else {
    addLogs($_REQUEST['q'], $GLOBALS['tenant_id'], "Stage id not found. Details:" . json_encode($data));
}
...
q("update sms_sequence set deal_amount='".$deal_amount."',hold_till_date='".date("Y-m-d",strtotime($deal_info['data']['c96027b10f722b45d43a1821b25b192528934972']))."' where tenant_id='".$GLOBALS['tenant_id']."' AND last_deal_id='".$pipedrive_id."'");
q("update sms_sequence_app_out set hold_till_date='".date("Y-m-d",strtotime($deal_info['data']['c96027b10f722b45d43a1821b25b192528934972']))."' where tenant_id='".$GLOBALS['tenant_id']."' AND last_deal_id='".$pipedrive_id."'");
...
$tbl_camp_data = qs("select * from active_campaign_contact where  tenant_id='".$GLOBALS['tenant_id']."' AND last_deal_id='$pipedrive_id'");
$ac_data = array();
$ac_data['email'] = $email;
...
if (!empty($tbl_camp_data)) {
    if ($tbl_camp_data['last_stage_id'] == $pipedrive_stage) {
        addLogs($_REQUEST['q'], $GLOBALS['tenant_id'], "Stage id not changed");
    }else{
        if ($pipedrive_stage == '3' && in_array($deal_source, array('44','37'))){
            ...
            addLogs($_REQUEST['q'], $GLOBALS['tenant_id'], "App Out Email is set for deal ".$pipedrive_id);
        }
