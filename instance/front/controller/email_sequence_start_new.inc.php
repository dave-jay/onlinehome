<?php
_errors_on();
$all_tenants = q("select * from admin_users where is_active='1' and id=2");
foreach ($all_tenants as $each_tenant):
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL) === false) {
        ...
        $seq_detail = qs("select * from email_seq_time where tenant_id='{$GLOBALS['tenant_id']}' and sequence_name='day1_1_sent'");
        $next_seq = 'day1_1_sent';
        
        $deal_id = $pipedrive_id;
        $mail = $seq_detail['email_text'];
        $mail = str_ireplace("[AGENTS NAME]", $agent_fname, $mail);
        $mail = str_ireplace("[MERCHANTS NAME]", $fname, $mail);        
        $subject = $seq_detail['email_subject'];
        addLogs($_REQUEST['q'], $GLOBALS['tenant_id'],"Email: Trying to send email on " . $email);
        try {
            $apiCore = new apiCore();
            $apiCore->doCall("http://45.79.140.218/lysoft/hook_email", array("to" => $email, "subject" => $subject, "content" => $mail, "mail_from_email" => $agent_email, "password" => $agent_pass, "mail_from_name" => $agent_name, "bcc" => "solarquotes+deal$pipedrive_id@pipedrivemail.com"), "POST");
