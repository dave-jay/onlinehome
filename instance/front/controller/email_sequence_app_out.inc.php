...
foreach ($all_tenants as $each_tenant):
    ...
    $email_sequence_data = q("select * from email_sequence_app_out where tenant_id='{$GLOBALS['tenant_id']}' AND need_to_send_email='1'");
    foreach ($email_sequence_data as $each_email) {
        ...
        if ($req_email_detail['success'] == 1) {
            if (IsTimeToSendEmail($GLOBALS['tenant_id'], strtotime($each_email['modified_at']), $req_email_detail['next_seq'], $each_email['timezone'], $each_email['hold_till_date'], "_app_out")) {
