...
class Call_distribution {
    ...
    public static function getUserList() {
        return q("SELECT * FROM pd_users WHERE is_deleted = 0 AND is_active='1' AND tenant_id='{$_SESSION['user']['tenant_id']}' ORDER BY name ASC");
    }

    public static function AllLeadSorceNull() {
        unset($fields);
        $fields["pd_user_id"] = '';
        return qu("call_list_by_source", $fields, " 1 = 1 AND tenant_id='{$_SESSION['user']['tenant_id']}' ");
    }

    public static function CheckSourceID($source_id) {
        $res = qs("SELECT count(id) as total_record FROM call_list_by_source WHERE pd_source_id = '{$source_id}' AND tenant_id='{$_SESSION['user']['tenant_id']}'");
        return $res['total_record'];
    }

    public static function GetSourceUserIdList($source_id) {
        return qs("SELECT * FROM call_list_by_source WHERE pd_source_id = '{$source_id}' AND tenant_id='{$_SESSION['user']['tenant_id']}'");
    }
